...
class Points(commands.Cog):
    ...
    async def points(self,ctx,user: discord.User = None):
        ...
        async with ctx.channel.typing():
            ...
            points = 0 if (points==None) else points.get("points")
    ...
    async def top(self,ctx,value=1):
        ...
        text = []
        value = len(list(data)) if (value>len(list(data))) else value
            ...
            async with ctx.channel.typing():
                ...
                try:
                    for user in users:
                        ...
                        old_points = 0 if (old_points==None) else old_points.get("points")
                        if old_points == 0:
                            db.insert_one({"chr_id" : user.id,"name" : str(user.name)+f"#{user.discriminator}","points" : points})
                        else:
                            db.update_one({"chr_id" : user.id}, {"$set" : {"points" : old_points+points}})
                        await user.send(f'''You were given `{points}` Points by {ctx.author}''' + str("" if reason=="" else f"for {reason}"))
                    embed = discord.Embed(title = "Points Update",color = discord.Color.green())
                    embed.add_field(name="Points given",value=points)
                    embed.add_field(name="User(s)",value=f"{' , '.join([x.display_name for x in users])}")
            ...
            try:
                for user in users:
                    ...
                    if old_points == 0:
                        ...
                        db.update_one({"chr_id" : user.id}, {"$set" : {"points" : old_points-points}})
                    await user.send(f'''Your points were reduced by `{points}` by {ctx.author}''' + str("" if reason=="" else f"for {reason}"))
                embed = discord.Embed(title = "Points Update",color = discord.Color.green())
    ...
    async def addpointsforattendence(self,ctx,points=5):
        ...
        async with ctx.channel.typing():
            points = int(points)
            voice_channel = discord.utils.get(ctx.guild.voice_channels, id=999239209263046697)
            ...
            try:
                for user in members:
                    ...
                    if old_points == 0:
                        ...
                        db.update_one({"chr_id" : user.id}, {"$set" : {"points" : old_points+points}})
                    await user.send(f'''You were given `{points}` Points by {ctx.author} for attending the meet.''')
                embed = discord.Embed(title = "Points Update",color = discord.Color.green())
                embed.add_field(name="Points given",value=points)
                embed.add_field(name="User(s)",value=f"{' , '.join([x.display_name for x in members])}")
    ...
    async def news_point(self,ctx,message_id):
        ...
        try:
            ...
            if old_points == 0:
                ...
                db.update_one({"chr_id" : user.id}, {"$set" : {"points" : old_points+points}})
            await user.send(f'''You were given `{points}` Points by {ctx.author} for news sharing.''')
            await LOG_CHANNEL.send(f'''{user} were given `{points}` Points by {ctx.author} for news sharing.''')
        except:
            await ctx.send("Oh No! The code broke. `contact admin`.") 
